...
interface DataTableIntervalProps<TData extends HasId> {
  data: TData[];
...
function DataTableInterval<TData extends HasId>({
  data,
  columns,
  modalContent,
  onSaveData,
}: DataTableIntervalProps<TData>) {
    ...
    <>
      <div className="flex items-center justify-between py-4">
        <Input
          ...
          value={
            (table.getColumn("interval")?.getFilterValue() as string) ?? ""
          }
          onChange={(event) =>
            table.getColumn("interval")?.setFilterValue(event.target.value)
          }
          className="max-w-sm"
        />
        <Button
      ...
      <Dialog open={isModalOpen} onOpenChange={handleCloseModal}>
        <DialogContent className="md:max-w-[800px]">
          <DialogTitle>Interval</DialogTitle>
...
export default DataTableInterval;
